...
def loc2bbox(src_bbox, loc):
    ...
    src_ctr_y = torch.unsqueeze(src_bbox[:, 1], -1) + 0.5 * src_height    # 转化为中心和宽高
    ...
    ctr_x = dx * src_width + src_ctr_x         # 平移
    ctr_y = dy * src_height + src_ctr_y
    w = torch.exp(dw) * src_width              # 缩放
    h = torch.exp(dh) * src_height
    dst_bbox = torch.zeros_like(loc)
    dst_bbox[:, 0::4] = ctr_x - 0.5 * w
    dst_bbox[:, 1::4] = ctr_y - 0.5 * h
    dst_bbox[:, 2::4] = ctr_x + 0.5 * w
    dst_bbox[:, 3::4] = ctr_y + 0.5 * h        # 转化为左上角右下角的做坐标格式
...
class DecodeBox():
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, height, width, nms_iou, score_thresh):
        roi_cls_loc = (roi_cls_locs * self.std + self.mean)     # 标准化
